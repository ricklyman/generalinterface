...
Ba=null,Ra=null;if(this.minExponent!=null){Ba=this.minExponent;}else if(this.baseAtZero)Ba=0;if(this.maxExponent!=null)Ra=this.maxExponent;O=O*b.WM;da=da*b.WM;if(Ba==null)if(O==0)Ba=0;else Ba=Math.floor(Math.log(O)/Math.log(this.base));if(Ra==null)Ra=Math.ceil(Math.log(da)/Math.log(this.base));this.Rf(ub.a,Ba);this.Rf(ub.b,Ra);return true;};m.Li=function(f){var
x=this.md(ub.a);var
...
sb=Math.pow(this.base,Fb+1);return Ta+M*(sb-Ta)/this.majorDivisions;}};m.jg=function(){var
Ja=this.md(ub.a);var
Za=this.md(ub.b);var
sa=[];var
Ga=0;for(var
...
L=this.Li(f-1);var
Cb=this.Li(f);for(var
ra=1;ra<this.minorTickDivisions;ra++){var
kb=L+ra*(Cb-L)/this.minorTickDivisions;cb.push(this.getCoordinateFor(kb));}}return cb;};m.fh=function(){return false;};m.getCoordinateFor=function(p){var
Ua=this.md(ub.a);var
sa=this.md(ub.b);var
...
Na=Math.log(j)/Math.log(this.base);da=Math.round(this.length*(Na-Pa)/(yb-Pa));}return this.horizontal?da:this.length-da;};b.getVersion=function(){return jsx3.chart.Xe;};});
