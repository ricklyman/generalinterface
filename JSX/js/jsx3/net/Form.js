...
Lb=jsx3.util.Logger.getLogger(q.jsxclass.getName());q.lp=0;q.oS=250;q.t6=30000;q.METHOD_GET=ub.a;q.METHOD_POST=ub.b;q.sK=ub.c;q.kP=ub.d;q.EVENT_FILE_SELECTED=ub.e;q.EVENT_ON_RESPONSE=ub.f;q.EVENT_ON_ERROR=ub.g;q.EVENT_ON_TIMEOUT=ub.h;d.init=function(g,s,m){this.H0=ub.i+q.lp++;if(g==-1){this.XW(s);}else{this.XW();this.setMethod(g!=null?g:ub.a);this.setAction(s);this.setMultipart(m);}};q.newFromFragment=function(k){return new
...
xb=this;this.pq=window.setInterval(function(){xb.rN(++Ya<ma,M!==xb.LR.document);},s);};d.rN=function(j,k){try{this.LR.document.readyState==ub.C;}catch(Kb){window.clearInterval(this.pq);this.pq=null;this.Ns(jsx3._msg(ub.J,this,jsx3.NativeError.wrap(Kb)));return;}if(k&&(this.LR.document.readyState==ub.K||this.LR.document.readyState==ub.L)){window.clearInterval(this.pq);this.pq=null;this.nQ();}else if(!j){window.clearInterval(this.pq);this.pq=null;this.destroy();this.publish({subject:ub.h});}};d.Ns=function(r){this._form=null;this.publish({subject:ub.g,message:r});};d.abort=function(){window.clearInterval(this.pq);};d.getResponseText=function(){var
...
_a=this;this.LR.onload=function(){_a.G8();};try{this._form.submit();}catch(Kb){this.Ns((jsx3.NativeError.wrap(Kb)).toString());return;}this.pq=window.setTimeout(function(){_a.Fz();},j);};d.abort=function(){this.LR.onload=null;window.clearTimeout(this.pq);};d.Ns=function(p){this.LR.onload=null;window.clearTimeout(this.pq);this.publish({subject:ub.g,message:p});};d.G8=function(){this.LR.onload=null;if(this.pq){window.clearTimeout(this.pq);this.pq=null;}try{try{if(window.netscape&&netscape.security)netscape.security.PrivilegeManager.enablePrivilege(ub.N);}catch(Kb){}var
ta=this.LR.contentDocument;var
va=ub.D+ta.childNodes[0];}catch(Kb){this.publish({subject:ub.g,message:(jsx3.NativeError.wrap(Kb)).toString()});return;}this.nQ();};d.Fz=function(){this.LR.onload=null;this.pq=null;this.destroy();this.publish({subject:ub.h});};d.getResponseText=function(){var
